...
export const ExifPanel = React.memo(
  function ExifPanel({ exif, camera, lens }: ExifPanelProps) {
  ...
  (prevProps, nextProps) => {
    return (
      ...
      prevProps.exif?.camera === nextProps.exif?.camera &&
      prevProps.exif?.lens === nextProps.exif?.lens &&
      prevProps.exif?.aperture === nextProps.exif?.aperture &&
      prevProps.exif?.shutterSpeed === nextProps.exif?.shutterSpeed &&
      prevProps.exif?.iso === nextProps.exif?.iso &&
      prevProps.exif?.focalLength === nextProps.exif?.focalLength
    )
  }
)
